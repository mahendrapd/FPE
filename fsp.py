...
# nf=79
# X=abs(round(((X-X.min())/(X.max()-X.min())),2))
# print(X.head())
# for i in range(size):
#    for j in range(nf):
#        a[i][j]=float(a[i][j])
...
# trainfpe.to_csv('trainF.csv',header=False, index=False)
# print(trainfpe.head())
...
#featurerank.to_csv('trainRFE.csv', header=False, index=False)
#print(frank)
df=frank.sort_values(ascending=False)
idx=list(df.index)
#print(idx)
#print(idx[:39])
X_new = X.iloc[:, idx[:39]]
